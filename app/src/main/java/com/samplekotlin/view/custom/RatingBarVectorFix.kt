...
class RatingBarVectorFix @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = android.support.v7.appcompat.R.attr.ratingBarStyle) : RatingBar(context, attrs, defStyleAttr) {
    ...
    private fun init() {
        ...
        drawable.findDrawableByLayerId(android.R.id.background).setColorFilter(
                ContextCompat.getColor(context, android.R.color.darker_gray), PorterDuff.Mode.SRC_ATOP)
        drawable.findDrawableByLayerId(android.R.id.progress).setColorFilter(progressColor, PorterDuff.Mode.SRC_ATOP)
    ...
    private fun tileify(drawable: Drawable, clip: Boolean): Drawable {
        if (drawable is DrawableWrapper) {
            var inner: Drawable? = (drawable as DrawableWrapper).wrappedDrawable
            if (inner != null) {
                inner = tileify(inner, clip)
                (drawable as DrawableWrapper).wrappedDrawable = inner
            }
